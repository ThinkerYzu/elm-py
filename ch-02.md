...
這時，if 和 else 的區塊，需要更進一步縮排。
像這種外層包內層的情況，我們稱之巢狀 (nested)。
例如下列程式，
...
同樣的， while loop 也能有巢狀 (nested) 的結構，區塊內可以
有其它的 wile loop 或者 if-else。
...
給定參數 S 和 i，請問從數列第 S 個數字開始算起, S 後面的第 i 個數
字為何?
