...
例如，電話薄要以使用者的姓名，依序排列。
來自不同系統的帳單資料，可能得依目期排序，並列印。
排多非常的普遍，每一位 programmer 都必會幾種方法。

這裡介紹一個簡單的排序演算法(sorting algorithm)，稱為 selection sort。
基本概念，就是從頭到尾掃一次，找出最小(或最大)的一筆資料，然後把
該筆資料和第一筆資料對調。
接著從第二筆資料開始，再掃一次，找出剩餘最小(或最大)的一筆資料，
然後再對調到第二個位置。
接著第三小，第四小.... 宜到最後一筆。
...
第五和十一行是註解，所有井字號 (#) 後同一行的內容，都會被 Python 當成註解。
第八行， range(i, len(INPUT)) 傳回從 i 開幾，一直數到 len(INPUT) 的所有數字，
但不包括 len(INPUT) 本身。

第 6 ~ 14 行，從頭到每一直找出最大的 (biggest) 數字的位置，並在 12~14 行，
最大的值交換到第 i 個位置。
